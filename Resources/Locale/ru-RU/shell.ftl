...
shell-must-have-body = Для выполнения команды необходимо иметь тело.
...
shell-need-exactly-zero-arguments = Эта команда не требует аргументов.
shell-argument-uid = EntityUid

## Guards

shell-missing-required-permission = Для этой команды требуется право { $perm }!
...
shell-entity-target-lacks-component = Целевая сущность не имеет компонента { INDEFINITE($componentName) } { $componentName } компонента
